...
- Aim mode. Long-press the bar's handle, then tap anywhere for a mouse click, or drag for a mouse drag. In a browser a drag selects text instead of scrolling. Double-tap and drag to scroll with a finger instead, at your own speed, so a flick still flings. Long-press for a menu: a right click, Select text, which lifts the text under your finger into a panel where you can select part of it and copy, or Copy image, which puts the picture under your finger on the clipboard as it appears on screen. Both work where the app offers no copy at all, a chat message or a photo in it.
